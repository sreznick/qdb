...
std::string query::Expr::print(int depth) {
    ...
    if (_leftExpr != nullptr)
        left = _leftExpr->print(depth + 2);
    if (_rightExpr != nullptr)
        right = _rightExpr->print(depth + 2);

    if (_type == ExprType::PLUS) {
        op = "+";
    } else if (_type == ExprType::MINUS) {
        op = "-";
    } else if (_type == ExprType::UN_MINUS) {
        op = "-";
    } else if (_type == ExprType::MULTIPLY) {
        op = "*";
    } else if (_type == ExprType::DIVIDE) {
        op = "/";
    } else if (_type == ExprType::BRACKETS) {
        op = "()";
    } else if (_type == ExprType::INTEGER) {
        op = "INT: " + std::to_string(_integerField);
    } else if (_type == ExprType::EXPR_REAL) {
        op = "REAL: " + std::to_string(_floatField);
    } else if (_type == ExprType::STRING) {
        op = "TEXT: " + _stringField;
    } else if (_type == ExprType::FIELD) {
        op = "UNKNOWN";
    } else if (_type == ExprType::LOGICAL_EXPR) {
        return _logicalExpr->print(depth);
    }

    return left + std::string(depth, ' ') + op + '\n' + right;
}

std::string query::LogicalExpr::print(int depth) {
    std::string left = "", right = "", op = "";
    ...
    if (_rightExpr != nullptr)
        right = _rightExpr->print(depth + 2);
    if (_leftLogicalExpr != nullptr)
        left = _leftLogicalExpr->print(depth + 2);
    if (_rightLogicalExpr != nullptr)
        right = _rightLogicalExpr->print(depth + 2);

    if (_type == LogicalExprType::TRUE) {
        op = "BOOLEAN: TRUE";
    } else if (_type == LogicalExprType::FALSE) {
        op = "BOOLEAN: FALSE";
    } else if (_type == LogicalExprType::LOG_BRACKETS) {
        op = "()";
    } else if (_type == LogicalExprType::EQ) {
        op = "==";
    } else if (_type == LogicalExprType::NEQ) {
        op = "!=";
    } else if (_type == LogicalExprType::AND) {
        op = "AND";
    } else if (_type == LogicalExprType::OR) {
        op = "OR";
    } else if (_type == LogicalExprType::NOT) {
        op = "NOT";
    }
